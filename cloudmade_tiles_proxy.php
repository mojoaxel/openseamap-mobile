<?php
	error_reporting(0); 
	$stream = stream_context_create(array('http' => array('timeout' => 1))); 
	//$result = file_get_contents("http://c.tile.cloudmade.com/".$_GET['key']."/".$_GET['styleid']."/256/".$_GET['z']."/".$_GET['x']."/".$_GET['y'].".png", false, $stream); 
	$result = file_get_contents("http://a.tile.openstreetmap.de/tiles/osmde/".$_GET['z']."/".$_GET['x']."/".$_GET['y'].".png", false, $stream); 
	if ($result === false) { 
